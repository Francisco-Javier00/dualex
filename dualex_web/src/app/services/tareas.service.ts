...
export class TareasService {
   ...
   * 
   * @returns Un `Observable` con un array completo de objetos `TareaDTO`.
   */
  getTareas(): Observable<TareaDTO[]> {
    return this.http.get<TareaDTO[]>(`${this.API_URL}&m=listar`);
  }
   ...
   */
  getTareasByAlumno(alumnoId: number): Observable<TareaDTO[]> {
    return this.http.get<TareaDTO[]>(`${this.API_URL}&m=listarPorAlumno&idAlumno=${alumnoId}`);
  }
   ...
   * @returns Un `Observable` emitiendo la información de la `TareaDTO`.
   */
  getTareaById(id: number): Observable<TareaDTO> {
    return this.http.get<TareaDTO>(`${this.API_URL}&m=obtener&id=${id}`);
  }
   ...
   */
  createTarea(tarea: TareaDTO): Observable<TareaDTO> {
    return this.http.post<TareaDTO>(`${this.API_URL}&m=crear`, tarea);
  }

  /**
   * Modifica los datos de una tarea o calificación existente.
   * 
   * @param tarea El objeto `TareaDTO` que incluye el ID y los campos actualizados.
   * @returns Un `Observable` con la respuesta del servidor.
   */
  updateTarea(tarea: TareaDTO): Observable<TareaDTO> {
    return this.http.put<TareaDTO>(`${this.API_URL}&m=actualizar&id=${tarea.id}`, tarea);
  }
